...
#let see if its possible to rotate things how we like!
def norm(z, p):
    inverse_p = tf.divide(1,p)
    lp_power = tf.constant(p, dtype=tf.float32)
    
    return tf.reduce_mean(tf.pow(tf.reduce_sum(tf.pow(tf.abs(z),lp_power),axis=1),inverse_p))
...
loss_lp =  0.0005 * (norm(train_z, 0.5) - norm(train_z,2))
